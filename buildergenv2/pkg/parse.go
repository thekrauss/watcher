// Shamefully copy pasted from https://github.com/golang/mock/blob/master/mockgen/parse.go with some adjustements.
...
func (p *fileParser) parseFile(importPath string, file *ast.File) (*model.Package, error) {
	...
	for ns := range iterStructs(file) {
		s, err := p.parseStruct(ns.name.String(), importPath, ns.t)
		if err != nil {
			return nil, err
		}
...
func (p *fileParser) parsePackage(path string) (*fileParser, error) { //nolint:unused
	newP := &fileParser{
	...
	imp, err := build.Import(path, newP.srcDir, build.FindOnly)
	if err != nil {
		return nil, err
	}

	pkgs, err := parser.ParseDir(newP.fileSet, imp.Dir, nil, 0)
	if err != nil {
		return nil, err
	}
...
func (p *fileParser) parseStruct(name, pkg string, is *ast.StructType) (*model.Struct, error) {
	vp, err := p.parseFieldList(pkg, is.Fields.List)
	if err != nil {
		return nil, err
	}
...
func (p *fileParser) parseInterface(name, pkg string, it *ast.InterfaceType) (*model.Interface, error) { //nolint:unused
	...
	for _, field := range it.Methods.List {
		...
		case *ast.SelectorExpr:
			...
			filePkg, sel := v.X.(*ast.Ident).String(), v.Sel.String() //nolint:errcheck
...
func (p *fileParser) parseFieldList(pkg string, fields []*ast.Field) ([]*model.Parameter, error) {
	...
	for _, f := range fields {
		...
		for _, name := range f.Names {
			...
			ps[i] = &model.Parameter{Name: name.Name, Type: t, Tags: tag}
			i++
		}
	}
	return ps, nil
...
func (p *fileParser) parseType(pkg string, typ ast.Expr) (model.Type, error) {
	...
	case *ast.ArrayType:
		...
		if v.Len != nil {
			...
			case (*ast.Ident):
				...
				value = val.Obj.Decl.(*ast.ValueSpec).Values[0].(*ast.BasicLit).Value //nolint:errcheck
			}
	...
	case *ast.Ident:
		if v.IsExported() {
			...
			if ok {
				...
				return &model.NamedType{Package: pkg, Type: v.Name, HasFromStringMethod: hasFromStringMethod(maybeImportedPkg.Path(), v.Name)}, nil
			}
			// assume type in this package
			return &model.NamedType{Package: pkg, Type: v.Name, HasFromStringMethod: hasFromStringMethod(pkg, v.Name)}, nil
		}
	...
	case *ast.SelectorExpr:
		pkgName := v.X.(*ast.Ident).String() //nolint:errcheck
		pkg, ok := p.imports[pkgName]
		if !ok {
			return nil, p.errorf(v.Pos(), "unknown package %q", pkgName)
		}
		return &model.NamedType{Package: pkg.Path(), Type: v.Sel.String(), HasFromStringMethod: hasFromStringMethod(pkg.Path(), v.Sel.Name)}, nil
	case *ast.StarExpr:
...
type cacheStruct struct {
	hasFromStringMethodCache map[string]bool
	packageLoadCache         map[string][]*packages.Package
	mutex                    sync.RWMutex
}

func (c *cacheStruct) GetHasFromStringCache(path string, structName string) (bool, bool) {
	cacheKey := fmt.Sprintf("%s.%s", path, structName)
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	res, ok := c.hasFromStringMethodCache[cacheKey]
	return res, ok
}

func (c *cacheStruct) SetHasFromStringMethodCache(path string, structName string, hasFromStringMethod bool) {
	cacheKey := fmt.Sprintf("%s.%s", path, structName)
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.hasFromStringMethodCache[cacheKey] = hasFromStringMethod
}

func (c *cacheStruct) GetPackageLoad(path string) ([]*packages.Package, bool) {
	c.mutex.RLock()
	defer c.mutex.RUnlock()
	res, ok := c.packageLoadCache[path]
	return res, ok
}

func (c *cacheStruct) SetPackageLoad(path string, val []*packages.Package) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.packageLoadCache[path] = val
}
...
func hasFromStringMethod(pkg string, name string) bool {
	if strings.HasPrefix(pkg, "stash.ovh.net/cloud/integration/pkg/domain") {
		if res, ok := cache.GetHasFromStringCache(pkg, name); ok {
			return res
		}
		// Check if type have method FromString to permit to create specific builder with String instead of complex types
		cfg := &packages.Config{Mode: packages.NeedTypes | packages.NeedImports}
		pkgs, ok := cache.GetPackageLoad(pkg)
		if !ok {
			var err error
			pkgs, err = packages.Load(cfg, pkg)
			if err != nil {
				go cache.SetHasFromStringMethodCache(pkg, name, false)
				return false
			}
			go cache.SetPackageLoad(pkg, pkgs)
		}
		...
		// Look for the type in the package's scope
		obj := pkgs[0].Types.Scope().Lookup(name)
		if obj == nil {
			go cache.SetHasFromStringMethodCache(pkg, name, false)
			return false
		}
		if named, ok := obj.Type().(*types.Named); ok { // Check if it's a named type
			for i := 0; i < named.NumMethods(); i++ {
				method := named.Method(i)
				if method.Name() == "FromString" {
					go cache.SetHasFromStringMethodCache(pkg, name, true)
					return true
				}
			}
		}
...
func importsOfFile(file *ast.File) (normalImports map[string]importedPackage, dotImports []string, err error) {
	var (
		importPaths  []string
		packagesName map[string]string
	)
...
func iterStructs(file *ast.File) <-chan namedStruct {
...
func packageNameOfDir(srcDir string) (string, error) { //nolint:unused
...
func CreatePackageMap(importPaths []string) (map[string]string, error) {
	...
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("fail to run command: %w\n%s\n%s", err, cmd, stderr.String())
	}
